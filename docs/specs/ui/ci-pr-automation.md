...
close. Users and task agents need one control plane that keeps each linked PR
moving throughout its lifecycle without conflating state from sibling PRs.
...
The runtime branch and remote identity rules are defined by [ADR-2026-08-09-runtime-branch-remote-identity](../../decisions/2026-08-09-runtime-branch-remote-identity.md), extended by [ADR-2026-08-12-role-based-git-remotes](../../decisions/2026-08-12-role-based-git-remotes.md).

## What

- The PR CI popover above the chat input shows five automation controls for the selected linked PR:
...
- `PR events` is presentation only. Its switches retain the selected linked
  PR's independent state and remain reachable on desktop and mobile. When any
  of its three options is enabled, the section opens so active automation is
  not concealed.
...
- For tasks with multiple linked PRs, each automation switch is scoped by
  `(task_id, repository_id, pr_number)`. Enabling a switch in one PR's tab has
  no effect on another linked PR. A legacy update that omits PR identity fans
  the requested switch change out to every currently linked PR; a newly linked
  PR starts with all five switches off. The auto-fix prompt override and
  resolved reviewer login remain task-level, while dedupe, last-attempt,
  review-request, and terminal state remain per linked PR.
- When a worktree surface needs one selected linked PR, Kandev first matches the worktree's repository/branch context and exact writable-action source identity. Exactly one source match is selected; zero or multiple exact matches leave the worktree-specific selection unresolved. Kandev never chooses by linked-row order, PR number, branch name alone, or canonical base identity. This selection rule does not narrow task automation, which continues to evaluate every linked PR independently.
...
- Branch-only PR discovery uses the current branch's writable action head as its exact source identity. The shared remote-role resolver derives that identity from Git's resolved push destination; Kandev does not substitute the tracking upstream or comparison target when the action head cannot be resolved. Kandev never assigns behavior to the literal names `origin` or `upstream`.
...
- The task's attached repository remains the authorization and association anchor. Runtime Git configuration supplies only a credential-free observed head repository identity; it never rewrites the persisted repository's provider identity. A discovered PR must connect that attached repository and the exact observed head ref, and its base repository must remain inside the workspace GitHub scope and automation principal's access. Read-only discovery may use a known action identity even when its remote-head observation is unknown, but no mutation is authorized from unknown evidence.
...
- When no usable writable-action identity is available, Kandev falls back to the attached repository and literal local branch only for same-repository read-only discovery. A same-named branch in another fork is never enough to select a PR, multiple eligible PRs remain unlinked as ambiguous, and Push/Create PR remain unavailable until exact action-head evidence exists.
...
- `head_host` string, default `""`. Normalized host of the current branch's
  observed writable action head while the watch is searching.
...
- Branch mutation clears all `head_*` fields atomically before a new head is
  observed. Terminal reset of the same local branch retains its exact head.
- Every searching path resolves the attached authorization anchor through
  `repository_id`; mutable canonical base owner/repository fields are never
  reused as the anchor after reset.
- These GitHub-specific columns are the durable projection of a complete provider-neutral writable-source identity. They are written atomically and never populated by combining a tracking branch with the attached or canonical-base repository.
...
| Writable action identity cannot be normalized                          | Use the attached repository only for same-repository read-only discovery, never persist or log a credential-bearing URL, leave cross-fork candidates unlinked, and keep source-specific mutations unavailable.                                                  |
| Writable action ref is authoritatively absent                          | Keep the watch searching without linking an existing PR for that source ref; first Push remains governed by action-head identity, local contribution evidence, and generation-bound mutation authorization.                                                   |
...
| Multiple linked PRs have no unique exact source match for the worktree | Leave the worktree-specific selected PR unresolved while continuing task-level automation for each linked PR independently.                                                                                                                                   |
...
- **GIVEN** the current branch has no usable writable action identity,
...
- **GIVEN** the current branch has a tracking upstream but Git resolves no writable action head, **WHEN** branch-only PR discovery and remote controls evaluate it, **THEN** Kandev does not reinterpret the tracking ref as the PR source or Push target; only attached-repository read-only discovery remains eligible.
- **GIVEN** two searching watches share an attached repository and local branch
  but have different exact heads, **WHEN** batched discovery runs, **THEN** each
  watch consumes only the result keyed by its own complete head identity.
- **GIVEN** more than two sibling forks have open pull requests with the same
  branch name, **WHEN** the exact eligible head appears after the first result
  page, **THEN** discovery continues until it can link that one candidate or
  prove ambiguity.
- **GIVEN** GitHub changes only the letter casing of an owner or repository,
  **WHEN** discovery compares identities, **THEN** the repository still matches
  while a branch-name case difference remains distinct.
- **GIVEN** push detection observes a writable head and an intervening
  searching watch appears before retry completion, **WHEN** the PR is still not
  available, **THEN** Kandev persists the observed head on that watch for later
  polling and restart.
- **GIVEN** a searching watch changes local branches, **WHEN** the mutation is
  committed, **THEN** the prior branch's head identity is cleared atomically;
  resetting a terminal PR on the same branch retains its exact head.
- **GIVEN** a watch previously resolved to a PR base different from its
  attached repository, **WHEN** it returns to searching, **THEN** every direct
  and batched lookup authorizes through the immutable `repository_id` anchor.
...
- **GIVEN** a task has two open linked PRs, **WHEN** the user enables an automation control from the first PR's tab, **THEN** only that PR becomes eligible for automation, the second PR's tab remains unchanged, and each PR records its own last-fix and last-merge state.
- **GIVEN** a task has two linked PRs in different repositories with the same PR number, **WHEN** the user enables a switch for one, **THEN** the other PR's switch is unaffected because the switch identity includes `(task_id, repository_id, pr_number)`.
- **GIVEN** a task has two linked PRs, **WHEN** an agent calls `update_task_pr_automation_kandev` naming one PR's `repository_id` and `pr_number`, **THEN** only that PR's switches change.
- **GIVEN** a task has two linked PRs, **WHEN** an agent calls `update_task_pr_automation_kandev` without `repository_id` or `pr_number`, **THEN** the requested switch changes on every currently linked PR.
- **GIVEN** a task has two linked PRs, **WHEN** the user disables auto-fix on one PR while it stays enabled on the other, **THEN** re-enabling it on the first PR resets only that PR's round count, exhaustion, and fix checkpoint.
- **GIVEN** automation was configured before per-PR scoping shipped, **WHEN** the backend boots for the first time after upgrade, **THEN** every PR linked at boot inherits the legacy task-level values, while a PR linked afterward starts with all five switches off.
- **GIVEN** automation was migrated to per-PR scoping and a user turned a switch off for one PR, **WHEN** the backend boots again, **THEN** that PR's switch remains off and migration is not replayed.
- **GIVEN** a task has two linked PRs but only one exact source repository/ref matches the current worktree's writable action head, **WHEN** the PR CI surface selects a worktree-specific PR, **THEN** it selects that PR while automation continues to evaluate both.
- **GIVEN** a task has multiple linked PRs and none or more than one exactly matches the current worktree's writable action head, **WHEN** the PR CI surface needs one worktree-specific PR, **THEN** selection remains unresolved instead of choosing the first association or a same-named branch.
