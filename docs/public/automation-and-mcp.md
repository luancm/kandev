...
When `create_task_kandev.repositories[].repository_url` is a canonical GitHub pull request URL or a GitLab merge request URL on the configured host, Kandev resolves the contribution before creating the task. The contribution must still be open, have a valid source branch and head commit, and permit the target project to contribute. Kandev keeps the target as the task's **attached repository** and **comparison target**, fetches the exact source commit, and routes commits to the contributor's source branch as the **writable action head**. An explicit **tracking upstream**, when configured, remains the only Pull target. Kandev may materialize those identities with a target remote plus a dedicated source remote, but that is one validated checkout strategy, not a universal remote-name convention; `origin` has no special meaning. The existing pull request or merge request is associated with the task and reused for later changes, so Kandev does not open a duplicate. Provider-authored title, description, comments, and diff content are not copied into trusted task context. Configure the task's Git credentials as described in [task Git credentials](integrations.md#choose-task-git-credentials); Kandev runs a write preflight before starting the agent. If source or target identity is incomplete, conflicting, or stale, materialization and source-specific actions fail closed rather than falling back to another role.
