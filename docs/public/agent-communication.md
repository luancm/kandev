...
The destination and options are validated together. `instructions` is appended after the destination step's normal prompt, and the profile, model, and reset values do not change workflow defaults. A move requested while the current agent is running returns `disposition: "deferred"` and applies after the turn ends; an idle move returns `disposition: "committed"`. The legacy top-level `prompt` argument remains accepted as an alias for `entry_options.instructions`, but sending both different non-empty values is a validation error. Pull-request draft/readiness is not a generic move option.
