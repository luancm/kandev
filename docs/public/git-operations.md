...
- **Old Git push failure remains visible after repair:** Kandev keeps the diagnostic record but removes its active error card and Fix action after a matching successful push or a fresh status observation proves the current upstream branch is synchronized. A failed or ambiguous retry remains actionable.
