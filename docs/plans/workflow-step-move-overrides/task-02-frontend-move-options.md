...
- A stepper target can be moved directly or opened in the options form. The form submits the selected target plus reset_context, instructions, agent_profile_id, and model with no live session mutation before submission.
- Chat and passthrough next-step controls have equivalent direct and sidecar-options behavior and target the same next step.
- Agent profile and model suggestions use existing store/capability data and communicate that the values are for this move only; instructions copy explains that it is appended to the target workflow prompt.
...
- Tests cover payload construction, direct versus options actions, profile/model/reset/prompt state, chat/passthrough parity, and touch Drawer rendering.
