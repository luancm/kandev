...
Integrated the freshly fetched `upstream/main` at `e014f8072f9d9535525a2a5cd9d72d3c06333e36` with merge commit `cbf6795551060b8dc16ea00c2371da3d42aba7bc`; `git merge-base --is-ancestor upstream/main HEAD` passes. The branch history was not rewritten.

The merge had five expected semantic conflicts: `apps/backend/internal/agentctl/types/streams/git.go`, `apps/backend/internal/agent/runtime/agentctl/git.go`, `apps/backend/internal/agent/runtime/lifecycle/events.go`, `apps/backend/internal/agentctl/server/api/git.go`, and `docs/specs/tasks/remote-contribution-tasks.md`. Resolution retains the role-aware `HeadRemote` contract and the upstream `RemoteHeadCommit` snapshot together, propagating both through the agentctl client, API single/multi-repository responses, lifecycle event publication, and frontend-compatible JSON. The remote-contribution spec retains the role-aware authorization/mutation rules and incorporates the upstream date and provider-history-unavailable scenario.

The clean-overlap review covered lifecycle/status tests, `workspace_git_status.go`, GitHub models/store/PAT code, frontend Git-status hydration, decision/spec indexes, and the CI automation spec. Upstream's unrelated changes are preserved in the merge commit. Focused checks passed: `cd apps/backend && go test -tags fts5 ./internal/agentctl/server/process ./internal/agentctl/server/api ./internal/agent/runtime/agentctl ./internal/agent/runtime/lifecycle ./internal/github ./internal/orchestrator ./internal/task/statussummary -count=1` (all seven packages), `cd apps && pnpm install --frozen-lockfile` (lockfile unchanged, exit 0), and `cd apps/web && pnpm exec vitest run lib/ws/handlers/git-status.test.ts lib/state/slices/session-runtime/git-status-multi-repo.test.ts hooks/domains/session/use-session-git-summary.test.ts` (3 files, 20 tests passed). `git diff --check` passes.

Post-merge path audit: all 198 existing paths listed in the `Files likely touched` sections of Tasks 00-12 are present; 22 absent paths are explicitly planned new production/test files for Tasks 01, 02, 03, 04, 05, and 06, not renames or upstream removals. No planned existing path was renamed or missing. The new API seam introduced by upstream is `RemoteHeadCommit` alongside the pre-existing role-aware `HeadRemote`; no task file or dependency ID changed. Ownership intersections are as follows: Task 02/03 share `service_pr_watch_batched.go`; Task 03/04 share GitHub watch/store files; Task 03/05 share `event_handlers_github.go`; Task 06/07 share `agentctl` client/API Git files. These are dependency-ordered overlaps, so Tasks 06 and 07 must execute sequentially despite the plan's parallel-safe label until the primary reassigns or confirms that boundary. Task 00 also necessarily overlaps the status/provider files it audited. The primary can release Task 01 after reconciling this audit; no upstream path drift blocks downstream work.
