...
Final review corrections reseed provider commit arrays after the moved head and assert the old and moved SHAs remain distinguishable on desktop and mobile. GitLab keeps the canonical target remote and MR URL while the live action remote is a local-bare-backed fork; GitHub push autolink now uses a reviewer-fork head against the canonical base. Mobile external-link coverage asserts the canonical renamed-old URL before reopening the modified file and checking the fork URL.
