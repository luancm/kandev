...
Corrective review pass: per-repository Pull/Rebase/Merge menus now fail closed for missing status, missing role evidence, unknown counts, and unresolved comparison scopes, while a selected contribution's Pull disablement no longer suppresses safe sibling repositories. Mobile top-bar metrics and recent-task sidebar diff totals aggregate repository-scoped status instead of whichever legacy status arrived last, with unresolved comparison totals failing closed. Provider-ahead contribution Pull now requires a non-empty role generation, exact provider source/tracking identity and head, known tracking counts, and complete provider ancestry; action-head and tracking failures remain independent. TaskPR source identities preserve complete cross-repository head metadata, and Create PR requires both complete action/source and comparison evidence. Added focused tests for omitted observations under a generation, complete identity/count validation, exact provider-head routing, sibling-menu safety, aggregation, and comparison-gated per-repo Rebase/Merge. Corrective evidence: 116 focused Vitest tests, `pnpm run typecheck`, `pnpm --filter @kandev/web lint`, `pnpm run i18n:check`, `pnpm run i18n:ratchet`, and `git diff --check` pass.

Second-review corrective pass: repository-scoped Changes push and Create PR callbacks now require complete per-repository evidence, mobile and desktop menus fail closed independently per repository, and single-repository Pull/Rebase/Merge paths return explicit failure results when tracking or comparison evidence is absent. Create PR now requires an exact writable source identity even for legacy status, while provider-ahead Pull requires exact source/tracking identity, head, generation, counts, and ancestry. Mutation callbacks reject unknown or headless present observations before sending a request, and hydration retains repository-scoped siblings across reconnect. Added focused coverage for these gates, mobile Create PR, blocked-operation failures, legacy source omission, and repository hydration. Second-review evidence: `(cd apps/web && pnpm exec vitest run hooks/domains/session/use-session-git-derived.test.ts hooks/domains/session/remote-contribution-relation.test.ts hooks/domains/session/use-session-git-grouping.test.ts hooks/use-git-operations.test.ts components/task/changes-panel-timeline-grouping.test.tsx components/task/mobile/session-mobile-top-bar-git-controls.test.tsx lib/state/hydration/hydrator.test.ts lib/ws/handlers/git-status.test.ts lib/state/slices/session-runtime/git-status-multi-repo.test.ts)` passes 124 tests; `NODE_OPTIONS=--max-old-space-size=4096 pnpm run typecheck`, `pnpm run i18n:check`, `pnpm run i18n:ratchet`, and `git diff --check` pass. Targeted ESLint reports no errors and no new warnings in the newly extracted safety components; the broader touched-file lint retains existing Task08 warnings documented in the prior Results paragraph.
