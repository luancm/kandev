...
| Provider association, sync, and removal events rehydrate every running execution without restart | `apps/backend/internal/backendapp/comparison_contexts_test.go`, `apps/backend/internal/orchestrator/event_handlers_github_test.go` | `cd apps/backend && go test -tags fts5 ./internal/orchestrator ./internal/backendapp -run 'Test.*(ComparisonContext|RemoteHead|GitHub)' -count=1` |
