...
describe("buildKanbanCardMenuEntries — move options", () => {
  it("nests move options under each direct step entry", () => {
    ...
    const directStep = directMenu.children.find(
      (entry) => entry.key === "step-step-2",
    );
    ...
    expect(onMoveToStepWithOptions).toHaveBeenCalledWith("step-2");
  });
  ...
  it("keeps the current step disabled when options are available", () => {
    ...
    expect(currentStep.children[0].kind === "item" && currentStep.children[0].disabled).toBe(
      true,
    );
  });
});
