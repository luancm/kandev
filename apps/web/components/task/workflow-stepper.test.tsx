...
function renderStepper() {
  render(<WorkflowStepper steps={STEPS} currentStepId={CURRENT_STEP_ID} taskId={TASK_ID} workflowId={WORKFLOW_ID} />);
}
...
describe("WorkflowStepper", () => {
  ...
  it("uses compact outer sizing for the current step popover", async () => {
    collapsedMock.mockReturnValue(false);
    render(<WorkflowStepper steps={STEPS} currentStepId={CURRENT_STEP_ID} />);
    ...
    expect(content?.classList.contains("w-auto")).toBe(true);
    expect(content?.classList.contains("p-0")).toBe(true);
  });

  it("forwards trigger props injected by the popover to the step trigger", async () => {
    collapsedMock.mockReturnValue(false);
    renderStepper();

    // Radix injects popover state and pointer handlers onto the asChild trigger; dropping
    // them breaks hover/focus activation. Pointer-enter must open the popover (openDelay=200ms).
    ...
    await waitFor(
      () => expect(screen.getByRole("button", { name: /move here/i })).toBeTruthy(),
      { timeout: 2_000 },
    );
  });
...
describe("WorkflowStepper — move behaviour", () => {
  ...
  it("submits the expanded disclosure draft as one-shot entry options", async () => {
    renderStepper();

    await openStepActions();
    fireEvent.click(screen.getByTestId("workflow-step-a-move-options"));
...
describe("WorkflowStepper — options disclosure", () => {
  it("pins the popover while the options disclosure is expanded", async () => {
    vi.useFakeTimers();
    try {
      render(
        <WorkflowStepper steps={STEPS} currentStepId={CURRENT_STEP_ID} taskId={TASK_ID} workflowId={WORKFLOW_ID} />,
      );
      const trigger = screen.getByTestId(STEP_SPEC_ID);
      fireEvent.pointerEnter(trigger);
