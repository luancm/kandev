...
func TestNormalizeEntryOptions_TrimsFieldsAndDoesNotMutateInput(t *testing.T) {
	...
	if input.Instructions != "  reproduce the checkout failure  " || input.AgentProfileID != " profile-qa " || input.Model != " gpt-5.6-sol " {
...
func TestNormalizeEntryOptions_UsesLegacyPromptAlias(t *testing.T) {
	...
	got, err = NormalizeEntryOptions(&EntryOptions{Instructions: "inspect the failing test"}, " inspect the failing test ")
	if err != nil {
		t.Fatalf("same legacy and nested prompt should be accepted: %v", err)
	}
	if got.Instructions != "inspect the failing test" {
		t.Fatalf("normalized instructions = %q", got.Instructions)
	}
}

func TestNormalizeEntryOptions_RejectsConflictingLegacyPromptWithoutLeakingValues(t *testing.T) {
	const secretNested = "nested private handoff"
	const secretLegacy = "legacy private handoff"
...
func TestValidateEntryOptions_AllowsStepChangeAndEmptyOptions(t *testing.T) {
	if err := ValidateEntryOptions(&EntryOptions{Model: "gpt-5"}, MoveChangeStep); err != nil {
...
func TestValidateEntryOptions_ErrorDoesNotEchoUnknownChangeValue(t *testing.T) {
	...
	err := ValidateEntryOptions(&EntryOptions{Model: "gpt-5"}, MoveChange(sensitiveChange))
	if err == nil {
